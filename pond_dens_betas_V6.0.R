...
# Propuesta para diseño adaptativo de planes de muestreo simple para atributos
# considerando el histórico de la calidad del Proveedor
...
calc_wr <- function(N_, n, c, alpha_b, beta_b, AQL, LTPD) {
  ...
  wrp_val <- integral(f = function(p) wr_p(p, n, c, alpha_b, beta_b, N_), xmin = 0, xmax = AQL, method = "Kron")
  wrc_val <- integral(f = function(p) wr_c(p, n, c, alpha_b, beta_b, N_), xmin = LTPD, xmax = 1, method = "Kron")
  wrt_val <- wrp_val + wrc_val
  
  return(c(WRP_val = wrp_val, WRC_val = wrc_val, WRT_val = wrt_val))
}

decode <- function(string, l1, l2){
...
fitness <- function(string, N, alpha, beta, AQL, LTPD, l1, l2){
  par <- decode(string, l1, l2)
  n <- par[1]
  c <- par[2]
...
p1 <- c(0.95, 0.75, 0.50, 0.25, 0.10, 0.05) #, 0.1, 0.3)
p2 <- c(1e-4, 0.05, 0.10, 0.25, 0.40, 0.05) #, 0.89, 0.69) # 1 - PA(LTPD) -> Beta (Riesgo del Consumidor)
...
for (esce in cases) { # esce <- 1 + esce
  ...
  plan_genetico <- ga(type = "binary", nBits = l1 + l2, fitness = fitness,
                      popSize = 200,maxiter = 200, run = 100, seed = 060722,
                      N = Esce[esce, 1], alpha = Esce[esce, 2],
                      beta = Esce[esce, 3], AQL = Esce[esce, 4],
                      LTPD = Esce[esce, 5], l1 = l1, l2 = l2
  )
  
  plan_ga <- decode(plan_genetico@solution, l1, l2)
  ...
  resultados_riesgo <- data.frame(
    ...
    Proveedor = c("Excelente", "Bueno", "Regular", "Malo", "Muy Malo","Acum_ZI", "Naive", "OTB_est"),# paste("otro", 1:(abs(5-length(p1))))),
    n_clasic = n_clasic,
    c_clasic = c_clasic,
  ...
  for (j in 1:dim(alpha_beta_params)[1]) { # j <-6 1 + j
    ...
    if (!is.na(n_opt_found)) {
      ...
      resultados_riesgo[j, "delta_n_k"] <- (n_opt_found - n_clasic)/n_clasic
      resultados_riesgo[j, "delta_n_ga"] <- (n_opt_found - n_ga)/n_ga
      ...
      resultados_riesgo[j, "delta_n_k"] <- NA
      resultados_riesgo[j, "delta_n_ga"] <- NA
    }
  }
