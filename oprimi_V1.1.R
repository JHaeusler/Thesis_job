...
# OBJETIVO: Minimizar L = RAT + lambda * (n / (c + 1)) * P(p en ZI)
# LAMBDA INTEGRADO: Calculado a partir de AQL, RQL, beta y N.
# MÉTODO: Búsqueda exhaustiva con bucles anidados.
# ==============================================================================
...
# Derivado de los parámetros del sistema: lambda = (AQL * beta) / (RQL * N)
SCALING_FACTOR_N <- ((AQL/RQL)*((beta)/N))
CO_clasic <- phyper(q = c_clasic, m = m_hyp, n = n_hyp, k = n_clasic, lower.tail = TRUE)

message(paste("FACTOR LAMBDA (INTEGRADO):", round(SCALING_FACTOR_N, 6), "(Equilibra RAT y Costo 'n')"))
...
optimizar_loss_combinada <- function(min_p, max_p, escenario) {
  ...
  # b. Calcular la Masa de Probabilidad en la Zona de Indiferencia (P(p en ZI))
  zi_prob_mass <- sum(CO_clasic[ind_zi] * dens_eval_p[ind_zi]) * delta_p
  
  message(paste("\nEscenario:", escenario))
  message(paste("  - Masa de Probabilidad en ZI [P(p en ZI)]:", round(zi_prob_mass, 6)))
  message("  - Buscando el plan que minimiza L = RAT + lambda * (n/(c+1))*P(p en ZI)...")
  ...
  for (n_actual in 1:n_clasic) {
    ...
    for (c_actual in 0:(n_actual - 1)) {
      ...
      # USO DEL FACTOR DE ESCALA (SCALING_FACTOR_N)
      penalizacion <- SCALING_FACTOR_N * (n_actual / (c_actual + 1)) * zi_prob_mass
  ...
  CO_clasic_eval <- phyper(q = c_clasic, m = m_hyp, n = n_hyp, k = n_clasic, lower.tail = TRUE)
  rp_clasic <- sum((1 - CO_clasic_eval[ind_aql]) * dens_eval_p[ind_aql]) * delta_p
  rc_clasic <- sum(CO_clasic_eval[ind_rql] * dens_eval_p[ind_rql]) * delta_p
  
  return(data.frame(
    Escenario = escenario,
    n_Clasico = n_clasic,
    c_Clasico = c_clasic,
    RAT_Clasico = rp_clasic + rc_clasic,
...
#-------------------------------------------------------------------------------
message("\n=======================================================================================")
message("  RESULTADOS FINALES: OPTIMIZACIÓN BAYESIANA CON PÉRDIDA COMBINADA (LAMBDA INTEGRADO)")
message("=======================================================================================")
message(paste("Factor de Escala (lambda):", round(SCALING_FACTOR_N, 6)))
message("Fórmula de Pérdida Minimizada (L): RAT + lambda * (n/(c+1)) * P(p en ZI)")
message("---------------------------------------------------------------------------------------")
