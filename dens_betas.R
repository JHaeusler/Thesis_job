...
# Función principal para calcular el Riesgo Ponderado Integrado (RP y RC)
calc_wr <- function(n, c, alpha_b, beta_b, AQL=0.05, LTPD=0.10) {
  
  # 1. Función para el integrando del Riesgo del Productor (RP)
  # Captura n, c, alpha_b, beta_b del entorno
  integrand_rp <- function(p) {
    f.p <- dbeta(p, alpha_b, beta_b)
    # wr_p: (1 - PA) * f(p)
    return( (1 - Pa(n, c, p)) * f.p )
  }
  
  # 2. Función para el integrando del Riesgo del Consumidor (RC)
  integrand_rc <- function(p) {
    f.p <- dbeta(p, alpha_b, beta_b)
    # wr_c: PA * f(p)
    return( Pa(n, c, p) * f.p )
  }
  
  # --- Cálculo con pracma::integral() ---
  
  # Riesgo del Productor (RP) Integrado [0, AQL]
  rp_val <- integral(f = integrand_rp, 
                     xmin = 0, xmax = AQL, 
                     method = "Kron") # Método de Gauss-Kronrod
  
  # Riesgo del Consumidor (RC) Integrado [LTPD, 1]
  rc_val <- integral(f = integrand_rc, 
                     xmin = LTPD, xmax = 1, 
                     method = "Kron")
