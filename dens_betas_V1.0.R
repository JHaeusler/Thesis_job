...
for (i in 1:5) {
  ...
  # --- II. Búsqueda del Plan Óptimo (Minimizar Riesgo Total Ponderado) para el Escenario i ---
  
  # Inicializar variables de búsqueda para cada escenario
  n_opt_found <- NA
  c_opt_found <- NA
  min_twr <- Inf # Riesgo Total Ponderado Mínimo
  
  # Búsqueda exhaustiva: Itera n de 1 hasta n_clasic para encontrar el plan que minimice el TWR
  for (n_ in 1:n_clasic) { 
    
    # c_ debe ser menor que n_ (el número máximo de defectuosos aceptables)
    for (c_ in 0:(n_ - 1)) {
      ...
      # Calcular el Riesgo Total Ponderado (TWR)
      twr_current <- risks_opt["RP_val"] + risks_opt["RC_val"]
      
      # Condición de Optimización (Minimiza TWR)
      if (twr_current < min_twr) {
        
        # Plan óptimo encontrado hasta ahora (el de menor TWR)
        min_twr <- twr_current
        n_opt_found <- n_
        c_opt_found <- c_
      }
  ...
  # El plan óptimo es el que minimiza el TWR dentro del rango n <= n_clasic
  resultados_riesgo[i, "n_opt"] <- n_opt_found
  resultados_riesgo[i, "c_opt"] <- c_opt_found
  
  # Recalcular y almacenar los riesgos óptimos para el n_opt, c_opt encontrado
  if (!is.na(n_opt_found)) {
    ...
    # Si n_opt_found es NA (solo ocurriría si n_clasic fuera 0 o 1)
