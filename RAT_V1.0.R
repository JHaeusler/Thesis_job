...
ejecutar_analisis_escenario <- function(nombre_escenario, p_min, p_max) {
  ...
  # 2.1. Definición de la Densidad del Escenario (f(p))
  dens_eval <- dunif(prop, min = p_min, max = p_max)
  
  # 2.2. Cálculo del Plan Clásico (Sigue siendo la referencia para los riesgos alpha/beta)
  # NOTA: Este plan es INDEPENDIENTE de f(p)
  ...
  # 2.3. Cálculo del RAT Clásico (Evaluado con la dens_eval del escenario)
  
  # Riesgo del Productor (RP) Ponderado
  rp_clasic <- sum((1 - CO_clasic[ind_aql]) * dens_eval[ind_aql] * delta_p)
  # Riesgo del Consumidor (RC) Ponderado
  rc_clasic <- sum(CO_clasic[ind_rql] * dens_eval[ind_rql] * delta_p)
  # RAT Clásico (WR_C)
  wr_c_scenario <- rp_clasic + rc_clasic
  
  cat(sprintf("Plan Clásico de Referencia: n=%d, c=%d\n", n_clasico, c_clasico))
  cat(sprintf("RAT Clásico (Evaluado con f(p)): %f\n", wr_c_scenario))
  ...
  # Filtramos la grilla para asegurar la validez: n >= c y n > 0
  ...
  mejor_plan <- NULL
  min_risk <- wr_c_scenario # Buscamos algo menor que el RAT Clásico
  found_better <- FALSE
  ...
  for (i_ in 1:nrow(space_busq_filt)) {
    ...
    # 3. Criterio de Parada: ¿El nuevo riesgo es mejor que el mínimo actual?
    if (wr_b < min_risk) {
      min_risk <- wr_b
  ...
  if (found_better) {
    ...
    cat(sprintf("RAT Secuencial: %f\n", min_risk))
    cat(sprintf("Mejora de Riesgo vs Clásico: %f\n", wr_c_scenario - min_risk))
    ...
    cat("\nNo se encontró un plan que mejore el RAT clásico en el espacio de búsqueda restringido (n <= n_clásico).\n")
  }
}
