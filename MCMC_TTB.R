...
# Fuente 3: Simulación Propia (Aislada de los vectores globales)
set.seed(060722)
N_sim <- 1000; AQL_sim <- 0.05; LTPD_sim <- 0.10; alpha_sim <- 0.01; beta_sim <- 0.05
plan_kier <- find.plan(PRP = c(AQL_sim, 1 - alpha_sim), CRP = c(LTPD_sim, beta_sim), N = N_sim, type = "hypergeom")
n_sample <- plan_kier$n; c_critico <- plan_kier$c

lotes <- 35; sum_n <- numeric(lotes); decis <- numeric(lotes)
for (k in 1:lotes) {
  L <- ifelse(runif(N_sim) <= AQL_sim, 1, 0)
  sample_n <- sample(L, n_sample)
  sum_n[k] <- sum(sample_n)
  decis[k] <- ifelse(sum_n[k] > c_critico, 1, 0)
}
...
# ============================================================================== #
# --- EXTRACCIÓN DE PARÁMETROS PARA LA FASE II (Master Script) ---
# ============================================================================== #
# Priors Gamma
a_mcmc_1 <- mean(res_gamma$samples[, 1])
b_mcmc_1 <- mean(res_gamma$samples[, 2])

# Priors Jeffreys
a_mcmc_2 <- mean(res_jeffrey$samples[, 1])
b_mcmc_2 <- mean(res_jeffrey$samples[, 2])

# Priors Bivariada (Torres, Tovar y Bran, 2025)
a_mcmc_3 <- mean(res_biv$samples[, 1])
b_mcmc_3 <- mean(res_biv$samples[, 2])

cat("\n>>> Parámetros MCMC extraídos exitosamente para la Fase II <<<\n")