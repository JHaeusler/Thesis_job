...
resultados_riesgo <- data.frame(
  ...
  RC_opt = NA,
  TWR_opt = NA, 
  # Ratios (Plan Óptimo Informado vs. Plan Clásico Naive)
  RP_Ratio = NA, 
  RC_Ratio = NA,
  TWR_Ratio = NA
)
...
for (i in 1:5) {
  ...
  for (n_ in 1:n_clasic) { 
    for (c_ in 0:(n_ - 1)) {
      ...
      # Minimizar el Riesgo Total Ponderado (TWR)
  ...
  if (!is.na(n_opt_found)) {
    ...
    # Cálculo de los ratios (Plan Óptimo Informado vs. Clásico Naive)
    # Ratio = Riesgo Opt / Riesgo Clásico Naive. Valores < 1 son ganancias.
    resultados_riesgo[i, "RP_Ratio"] <- resultados_riesgo[i, "RP_opt"] / resultados_riesgo[i, "RP_U01_Clasic"]
    resultados_riesgo[i, "RC_Ratio"] <- resultados_riesgo[i, "RC_opt"] / resultados_riesgo[i, "RC_U01_Clasic"]
    resultados_riesgo[i, "TWR_Ratio"] <- resultados_riesgo[i, "TWR_opt"] / resultados_riesgo[i, "TWR_U01_Clasic"]
    
  } else {
    resultados_riesgo[i, "TWR_opt"] <- NA
    resultados_riesgo[i, "RP_opt"] <- NA
    resultados_riesgo[i, "RC_opt"] <- NA
    resultados_riesgo[i, "RP_Ratio"] <- NA
    resultados_riesgo[i, "RC_Ratio"] <- NA
    resultados_riesgo[i, "TWR_Ratio"] <- NA
  }
}

# 5. Mostrar la tabla de resultados final
options(digits = 6) 
print("Tabla de Comparación de Riesgos y Ratios de Eficiencia: Naive vs. Bayesiano Óptimo")
print("------------------------------------------------------------------------------------")
...
resultados_final_comparado <- resultados_riesgo[, c(
  ...
  "TWR_U01_Clasic", "TWR_opt", "TWR_Ratio", # TWR (Total Risk) comparison
  "RP_U01_Clasic", "RP_opt", "RP_Ratio",
  "RC_U01_Clasic", "RC_opt", "RC_Ratio"
)]
