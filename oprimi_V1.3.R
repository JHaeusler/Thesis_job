...
#-------------------------------------------------------------------------------
# --- 3. Función de Optimización de Pérdida Combinada (L) [VERSIÓN CON COSTO INTRÍNSECO] ---
#-------------------------------------------------------------------------------
...
optimizar_loss_combinada <- function(min_p, max_p, escenario) {
  ...
  message(paste("\nEscenario:", escenario))
  message(paste(" - Riesgo de Aceptación ZI Plan Clásico (Ponderado):", round(zi_acceptance_risk_clasic, 6)))
  message(" - Buscando el plan que minimiza L = RAT + P_ZI_Severidad + P_Costo_Intrínseco...")
